...
abstract class Service implements ArrayAccess, JsonSerializable
{
     ...
     * @return string
     */
    public function filter(string $key, string $operator, $value)
    {
        $this->filter = "filter=[".$key."][".$operator."][".$value."]";
        return $this;
    }
     ...
     * @return string
     */
    public function getFullUrl($additional = null, $next = null)
    {
        $url = $this->full_url;
        if (substr($url, -1) != '/') {
            $url .= '/';
        ...
        if ($next != null)
            $additional = $additional.'/'.ltrim($next, '/');
    ...
    /**
     * Returns the full URL with additional concatinated and strings
     *
     * @return string
     */
    public function getStringUrl($additional = null, $next = null)
    {
        $string = (!empty($this->filter) && !empty($this->include)) ? '?'.$this->filter.'&'.$this->include : '?'.(($this->filter != '') ? $this->filter : $this->include);

        if (strlen($string) == 1)
            $string = trim($string, '?&');

        $url = trim($this->getFullUrl($additional, $next), '/') . $string;
    ...
    /**
     * Update a object and saves it in storage/API.
     *
     * @param  array $attributes
     *
     * @return self
    ...
    public function save()
    {
        if (isset($this->{$this->primaryKey})) {
            $data = $this->client->restfulRequest(strtolower($this->updateVerb),
                $this->getFullUrl($this->{$this->primaryKey}), (array)$this->toArray());
        } else {
    ...
    public function upload(array $attributes = [])
    {
        ...
        if (!isset($this->{$this->primaryKey})) {
            throw new FrilansInternationalException('Your request mismatch to required request.');
        }

        $data = $this->client->restfulRequest('post', $this->getFullUrl($this->{$this->primaryKey}).$this->uploadUri, (array)$this->toArray());
        $this->fill($data);
    ...
    public function find($id, string $slug = null)
    {
        if ($slug != '' && in_array($slug, $this->slugs))
            $data = $this->client->restfulRequest('get', $this->getStringUrl($id, $slug));
        else
            $data = $this->client->restfulRequest('get', $this->getStringUrl($id));
    ...
    {
        $response = $this->client->restfulRequest('delete', $this->getFullUrl($id));
        $this->fill($response);
        return $this;
    }
    ...
    {
        if (!empty($this->where)) {
            $data = $this->client->restfulRequest('get', $this->getStringUrl(), $this->where);
        } else {
            $data = $this->client->restfulRequest('get', $this->getStringUrl(), ['perpage' => $this->client->getPerPage()]);
        }
    ...
    public function first()
    {
        ...
        if (!empty($this->where)) {
            ...
            $data = $this->client->restfulRequest('get', $this->getStringUrl());
        }
        ...
        if ($data['count'] === 0) {
            throw new FrilansInternationalException('No Results Found');
        }
    ...
    public function all()
    {
        $payload = ['perpage' => $this->client->getPerPage()];
        $data = $this->client->restfulRequest('get', $this->getStringUrl(), $payload);
    ...
    public function collect(array $array, $cursor = [])
    {
        ...
        return new FrilansInternationalCollection($items);
    }
